...
_LISTING_PATH_RE = re.compile(r"/homedetails/|/b/")
...
def build_zillow_crawler(settings: Settings) -> PlaywrightCrawler:
    # NOTE: Zillow is heavily JS + anti-bot. This is a best-effort structure.
    ...
    async def handle(context: PlaywrightCrawlingContext) -> None:
        ...
        if not _is_listing_page(url):
            ...
            if unique:
                context.log.info(f"[zillow] Found {len(unique)} listing links")
                await context.enqueue_links(urls=unique)

            next_btn = await context.page.query_selector('a[aria-label="Next page"]')
            if next_btn:
                href = await next_btn.get_attribute("href")
                if href:
                    await context.enqueue_links(urls=[href])
            return
